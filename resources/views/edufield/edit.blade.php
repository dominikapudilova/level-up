            ...
            <form class="w-full " method="POST" action="{{ route('edufield.update', [ $edufield, 'course' => request('course') ]) }}">
                ...
                <div class="space-y-4 w-full">
                    <div>
                        ...
                        <x-input-text id="name" class="w-full mt-1" name="name" :value="old('name', $edufield->name)" autofocus></x-input-text>
                        <x-input-error :messages="$errors->get('name')" class="mt-2"/>
                    </div>

                    <div>
                        <x-input-label for="code_name">{{ __('Code name') }}</x-input-label>
                        <x-input-text id="code_name" class="w-full mt-1" name="code_name" :value="old('code_name', $edufield->code_name)"></x-input-text>
                        <x-input-error :messages="$errors->get('code_name')" class="mt-2"/>
                    </div>

                    <div>
                        <x-input-label for="description">{{ __('Description') }}</x-input-label>
                        <textarea id="description" name="description" class="w-full mt-1 rounded-md border-gray-200 focus:border-rose-300 focus:ring-rose-300/50 focus:ring-2 shadow-sm text-slate-600 text-sm">{{old('description', $edufield->description)}}</textarea>
