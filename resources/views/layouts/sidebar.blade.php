<nav
    class="fixed flex flex-col sm:static z-40 sm:bg-gray-100 bg-white sm:rounded-none rounded-xl sm:shadow-none shadow-2xl bottom-4 top-4 max-w-xs w-64 h-[calc(100vh-2rem)] transform sm:transform-none transition-transform duration-150 p-2"
...
>
    ...
    {{--                <x-nav-link class="" :href="route('dashboard')" :active="request()->routeIs('/')">{{ __('Configuration') }}</x-nav-link>--}}
        ...
        <div class="toggle-group mt-2">
            <input class="toggle-button" type="checkbox" id="show-photos" @checked(auth()->user()->showPhotos()) @change="setShowPhotos" />
            <label class="toggle-label whitespace-nowrap" for="show-photos">{{ __('Show photos') }}</label>
        </div>
    </div>
</nav>
...
<script>
    async function setShowPhotos() {
        let res = await fetch(`{{ route('profile.toggle-photos') }}`);
        if (!res.ok) {
            alert("{{ __('An error occurred while updating the setting.') }}");
        } else {
            location.reload();
        }
    }
